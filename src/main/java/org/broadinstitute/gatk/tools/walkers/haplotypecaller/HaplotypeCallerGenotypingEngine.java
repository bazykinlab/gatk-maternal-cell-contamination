...
public class HaplotypeCallerGenotypingEngine extends GenotypingEngine<AssemblyBasedCallerArgumentCollection> {
            ...
            if( loc >= activeRegionWindow.getStart() && loc <= activeRegionWindow.getStop() ) { // genotyping an event inside this active region
                ...
                final ReadLikelihoods<Allele> readAlleleLikelihoods = readLikelihoods.marginalize(alleleMapper,
                                                                                                  genomeLocParser.createPaddedGenomeLoc(genomeLocParser.createGenomeLoc(mergedVC),
                                                                                                                                           ALLELE_EXTENSION));
                System.out.println("after marginalization " + readAlleleLikelihoods.alleles());
                ...
                System.out.println("calculateGLsForThisEvent=" + genotypes);
    ...
    @Ensures("result != null")
        // TODO - can this be refactored? this is hard to follow!
    Map<Integer, ReadLikelihoods<Allele>> assignGenotypeLikelihoodsGiveMargs(final List<Haplotype> haplotypes,
                                               final ReadLikelihoods<Haplotype> readLikelihoods,
                                               final Map<String, List<GATKSAMRecord>> perSampleFilteredReadList,
                                               final byte[] ref,
                                               final GenomeLoc refLoc,
        ...
        final int ploidy = configuration.genotypeArgs.samplePloidy;
        final List<Allele> noCallAlleles = GATKVariantContextUtils.noCallAlleles(ploidy);
        HashMap<Integer, ReadLikelihoods<Allele>> out = new HashMap<>();
        for (final int loc : startPosKeySet) {
            if (loc >= activeRegionWindow.getStart() && loc <= activeRegionWindow.getStop()) { // genotyping an event inside this active region
                final List<VariantContext> eventsAtThisLoc = getVCsAtThisLocation(haplotypes, loc, activeAllelesToGenotype);
                ...
                out.put(new Integer(loc), readAlleleLikelihoods);
            }
        }
        return out;
    }
    ...
    private GenotypesContext  calculateGLsForThisEvent(final ReadLikelihoods<Allele> readLikelihoods, final List<Allele> noCallAlleles) {
        ...
        System.out.println("genotypingModel.calculateLikelihoods = " + genotypingModel);
        for (int s = 0; s < likelihoods.sampleCount(); s++)
            System.out.println(" likelihoods.sampleLikelihoods(s).getAsString() --  " + likelihoods.sampleLikelihoods(s).getAsString());
    ...
    public static Map<Allele, List<Haplotype>> createAlleleMapper( final Map<VariantContext, Allele> mergeMap, final Map<Event, List<Haplotype>> eventMap ) {
    ...
    public static Map<Event, List<Haplotype>> createEventMapper( final int loc, final List<VariantContext> eventsAtThisLoc, final List<Haplotype> haplotypes) {
